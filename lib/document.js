...
function flattenBlocks(blocks, context, answers, variables, output = []) {
  for (const block of blocks || []) {
    ...
    const runs = Array.isArray(block.runs) ? block.runs.map(run => ({ ...run, text: interpolate(run.text || '', context) })) : null;
    if (block.type === 'heading') output.push({ type: 'heading', text, runs });
    else if (block.type === 'paragraph' || block.type === 'conditional') output.push({ type: 'paragraph', text, runs });
...
async function generateDocx(form, answers, variables) {
  ...
  const children = blocks.map(block => new Paragraph({ heading: block.type === 'heading' ? HeadingLevel.HEADING_1 : undefined, spacing: { after: 180 }, children: (block.runs?.length ? block.runs : [{ text: block.text }]).map(run => new TextRun({ text: run.text || '', bold: Boolean(run.bold), italics: Boolean(run.italic), underline: run.underline ? {} : undefined, font: run.font || 'Arial' })) }));
